...
async function loadMe() {
  try {
    ...
    message.value = {
      type: "error",
      text: e?.userMessage || "Falha ao carregar perfil.",
    };
  }
}
...
async function saveChanges() {
  ...
  try {
    const payload = {};
    if (form.value.fullName !== user.value.fullName)
      payload.fullName = form.value.fullName;
    ...
    message.value = {
      type: "error",
      text: e?.userMessage || "Não foi possível salvar. Tente novamente.",
    };
  } finally {
    loading.value = false;
  }
...
<template>
  <div class="w-full">
    ...
    <section
      v-if="tab === 'perfil'"
      class="bg-white rounded-2xl shadow-sm ring-1 ring-slate-100"
    >
          ...
          <div>
            ...
            <div class="relative">
              <input
                ...
                class="w-full rounded-lg border-slate-200 pr-10 focus:border-sky-400 focus:ring-sky-400"
