  ...
  <div class="w-full">
    ...
    <div class="mb-4 grid grid-cols-1 md:grid-cols-4 gap-3">
      <div>
        ...
        <input type="date" v-model="filtros.inicio" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20"/>
      </div>
      <div>
        <label class="text-sm text-gray-700">Até</label>
        <input type="date" v-model="filtros.fim" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20"/>
      </div>
      <div>
        <label class="text-sm text-gray-700">Turno</label>
        <select v-model="filtros.turno" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      <div v-for="item in filtered" :key="item.id" class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <div class="flex items-start justify-between">
          <div>
            <div class="text-sm text-gray-500">{{ fmtData(item.data_producao) }} • {{ item.turno || '—' }}</div>
            <div class="mt-1 text-2xl font-semibold text-gray-900">{{ fmtNumero(item.quantidade_litros) }} L</div>
            <div class="mt-1 text-sm text-gray-700">Responsável: <span class="font-medium">{{ item.responsavel }}</span></div>
            <div class="mt-1 text-xs text-gray-600">Tipo: {{ item.tipo_leite || '—' }} • Qualidade: {{ item.qualidade || '—' }}</div>
            <div class="mt-1 text-xs text-gray-600">Equipamento: {{ item.equipamento_utilizado || '—' }} • Armazenamento: {{ item.local_armazenamento || '—' }}</div>
          </div>
        ...
        <div v-if="item.animais_contribuintes && item.animais_contribuintes.length" class="mt-2">
          <div class="text-xs text-gray-500">Animais contribuintes (IDs):</div>
          <div class="text-xs text-gray-700">{{ Array.isArray(item.animais_contribuintes) ? item.animais_contribuintes.join(', ') : item.animais_contribuintes }}</div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <div class="w-full max-w-3xl rounded-2xl bg-white shadow-xl">
        <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <h3 class="text-lg font-semibold text-gray-900">{{ isEditing ? 'Editar registro de leite' : 'Novo registro de leite' }}</h3>
          <button @click="close" class="rounded-md p-1.5 hover:bg-gray-50">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="h-5 w-5"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>
        ...
        <div class="max-h-[75vh] overflow-y-auto px-5 py-4">
          <section class="space-y-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="text-sm text-gray-700">Data da produção <span class="text-red-600">*</span></label>
                <input type="date" v-model="form.data_producao" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20"/>
                <p v-if="errors.data_producao" class="text-xs text-red-600 mt-1">{{ errors.data_producao }}</p>
              </div>
              <div>
                <label class="text-sm text-gray-700">Turno</label>
                <select v-model="form.turno" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20">
                  <option value="">Selecione</option>
                  <option v-for="t in TURNOS" :key="t" :value="t">{{ t }}</option>
                </select>
              </div>
              <div>
                <label class="text-sm text-gray-700">Quantidade (litros) <span class="text-red-600">*</span></label>
                <input type="number" step="0.01" min="0" v-model.number="form.quantidade_litros" placeholder="Ex.: 120" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20"/>
                <p v-if="errors.quantidade_litros" class="text-xs text-red-600 mt-1">{{ errors.quantidade_litros }}</p>
              </div>
              <div>
                <label class="text-sm text-gray-700">Responsável <span class="text-red-600">*</span></label>
                <input v-model="form.responsavel" placeholder="Ex.: João" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20"/>
                <p v-if="errors.responsavel" class="text-xs text-red-600 mt-1">{{ errors.responsavel }}</p>
              </div>
            </div>
          </section>
          ...
          <section class="space-y-3 mt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select v-model="form.tipo_leite" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20">
                  <option value="">Selecione</option>
              ...
              <div>
                ...
                <select v-model="form.qualidade" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20">
                  <option value="">Selecione</option>
              ...
              <div>
                ...
                <input type="number" step="0.1" v-model.number="form.temperatura" placeholder="Ex.: 3.5" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20"/>
              </div>
            </div>
          </section>
          ...
          <section class="space-y-3 mt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select v-model="form.equipamento_utilizado" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20">
                  <option value="">Selecione</option>
              ...
              <div>
                ...
                <select v-model="form.local_armazenamento" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20">
                  <option value="">Selecione</option>
              ...
              <div class="md:col-span-2">
                ...
                <div class="mt-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div class="rounded-md border border-gray-300 p-2 max-h-48 overflow-y-auto">
                    <div class="text-xs text-gray-500 mb-1">Selecione vacas</div>
                    <div v-if="loadingVacas" class="text-sm text-gray-500">Carregando...</div>
                    <div v-else class="space-y-1">
                      <label v-for="v in vacas" :key="v.id" class="flex items-center gap-2 text-sm">
                        <input type="checkbox" :value="v.id" v-model="form.animais_contribuintes" class="rounded border-gray-300"/>
                        <span class="text-gray-700">#{{ v.numero_animal }} — {{ v.nome_animal || 'sem nome' }}</span>
                      </label>
                    </div>
                  </div>
                  <div class="rounded-md border border-gray-300 p-2">
                    <div class="text-xs text-gray-500 mb-1">IDs selecionados</div>
                    <div class="text-sm text-gray-800 min-h-[2rem]">{{ form.animais_contribuintes.join(', ') || '—' }}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <section class="space-y-3 mt-6">
            ...
            <textarea v-model="form.observacao" rows="3" placeholder="Anotações relevantes" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20"></textarea>
          </section>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-5 py-4">
          ...
          <button :disabled="saving" @click="submit" class="rounded-lg bg-green-600 px-4 py-2 text-white hover:bg-green-700 disabled:opacity-60">
...
function validate() {
  ...
  if (form.value.quantidade_litros == null || Number(form.value.quantidade_litros) <= 0) {
    errors.value.quantidade_litros = 'Litros deve ser maior que 0.'; ok = false
  }
  if (!form.value.responsavel?.trim()) { errors.value.responsavel = 'Informe o responsável.'; ok = false }
  return ok
}
